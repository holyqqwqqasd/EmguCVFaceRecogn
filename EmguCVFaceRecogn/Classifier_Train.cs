...
public class Classifier_Train: IDisposable
{
    ...
    #region Public//Saving The Data
    public Image<Gray, Byte> PrepareImageToTrain(Image<Bgr, Byte> image, Rectangle faceRect)
    {
        Image<Gray, Byte> result;

        /*faceRect.X += (int)(faceRect.Height * 0.15);
        faceRect.Y += (int)(faceRect.Width * 0.22);
        faceRect.Height -= (int)(faceRect.Height * 0.3);
        faceRect.Width -= (int)(faceRect.Width * 0.35);*/

        result = image.Copy(faceRect).Convert<Gray, byte>().Resize(100, 100, Inter.Cubic);
        result._EqualizeHist();

        return result;
    }

    public bool AddImageForTrain(Image<Gray, Byte> image, string NAME_PERSON)
    {
        try
        {
            Image face_data = image.ToBitmap();
